...
def transformationNeck2(image:np.ndarray, per_to_side:float=0.8)->np.ndarray:
    ...
    def locate_side(image_:np.ndarray, x_:int, y_max:int) -> int:
        # 寻找 x=y, 且 y <= y_max 上从下往上第一个非 0 的点，如果没找到就返回 0
        y_ = 0
    ...
    def locate_width(image_:np.ndarray, y_:int, mode, left_or_right:int=None):
        # 从 y=y 这个水平线上寻找两边的非零点
        # 增加 left_or_right 的原因在于为下面 check_jaw 服务
        if mode==1:  # 左往右
            x_ = 0
    ...
    y_left_side = locate_side(image_=a_thresh, x_=x_left, y_max=y_low)  # 左边的点的 y 轴坐标
    y_right_side = locate_side(image_=a_thresh, x_=x_right, y_max=y_low)  # 右边的点的 y 轴坐标
...
def transformationNeck(image:np.ndarray, cutNeckHeight:int, neckBelow:int,
    ...
    # 在这里我们取消了对下巴的检查，原因在于输入的 imageHeight 并不能改变
    ...
    if per_to_side >1:
        assert ValueError("per_to_side 必须小于 1!")
    y_below = int((neckBelow - cutNeckHeight) * per_to_side + cutNeckHeight)  # 定位 y 轴坐标
...
def bestJunctionCheck_beta(image:np.ndarray, stepSize:int=4, if_per:bool=False):
    ...
    def locate_width_pro(image_:np.ndarray, y_:int, mode):
        ...
        x_, y_ 是启始点的坐标，每一次寻找都会让 y_+1
        mode==1 说明是找左边的边，即，image_[y_][x_] == 0 且 image_[y_][x_ + 1] !=0 时跳出;
            否则 当 image_[y_][x_] != 0 时，x_ - 1; 当 image_[y_][x_] == 0 且 image_[y_][x_ + 1] ==0 时 x_ + 1
        mode==2 说明是找右边的边，即，image_[y_][x_] == 0 且 image_[y_][x_ - 1] !=0 时跳出
            否则 当 image_[y_][x_] != 0 时，x_ + 1; 当 image_[y_][x_] == 0 且 image_[y_][x_ - 1] ==0 时 x_ - 1
        """
        y_ += 1
        if mode == 1:
...
def bestJunctionCheck(image:np.ndarray, offset:int, stepSize:int=4):
    ...
    与 beta 版不同的是它新增了一个阈值限定内容。
    对于脖子而言，我我们首先可以定位到上面的部分，然后根据上面的这个点向下进行遍历检测。
    与 beta 版类似，我们使用一个 stepSize 来用作斜率的检测
    但是对于遍历检测而言，与 beta 版不同的是，我们需要对遍历的地方进行一定的限制。
    ...
    height, width = a.shape
    ret, a_thresh = cv2.threshold(a, 127, 255, cv2.THRESH_BINARY)  # 将透明图层二值化
    # 直接返回脖子的位置信息，修正系数为 0, get_box_pro 内部也封装了二值化，所以直接输入原图
    y_high, y_low, _, _ = get_box_pro(image=image, model=1, correction_factor=0)
    # 真正有用的只有上下 y 轴的两个值...
    # 首先当然是确定起始点的位置，我们用同样的 scan 扫描函数进行行遍历。
    def scan(y_:int, max_num:int=2):
        num = 0
        # 设定两个值，分别代表脖子的左边和右边
        left = False
    ...
    for x_right_  in range(width -1 , -1, -1):
        if a_thresh[y_start][x_right_] != 0:
            x_right = x_right_
            break
    # 接下来我定义两个生成器，首先是脖子轮廓 (向下寻找的) 生成器，每进行一次 next，生成器会返回 y+1 的脖子轮廓点
    def contoursGenerator(image_:np.ndarray, y_:int, mode):
        """
        这会是一个生成器，用于生成脖子两边的轮廓
        y_ 是启始点的 y 坐标，每一次寻找都会让 y_+1
        mode==1 说明是找左边的边，即，image_[y_][x_] == 0 且 image_[y_][x_ + 1] !=0 时跳出;
            否则 当 image_[y_][x_] != 0 时，x_ - 1; 当 image_[y_][x_] == 0 且 image_[y_][x_ + 1] ==0 时 x_ + 1
        mode==2 说明是找右边的边，即，image_[y_][x_] == 0 且 image_[y_][x_ - 1] !=0 时跳出
            否则 当 image_[y_][x_] != 0 时，x_ + 1; 当 image_[y_][x_] == 0 且 image_[y_][x_ - 1] ==0 时 x_ - 1
        ...
        # 当处理失败则返回 False
            ...
            yield False
    # 然后是斜率生成器，这个生成器依赖子轮廓生成器，每一次生成轮廓后会计算斜率，另一个点的选取和 stepSize 有关
    def kGenerator(image_: np.ndarray, mode):
        """
        导数生成器，用来生成每一个点对应的导数
        """
        y_ = y_start
        # 对起始点建立一个生成器，mode=1 时是左边轮廓，mode=2 时是右边轮廓
        c_generator = contoursGenerator(image_=image_, y_=y_, mode=mode)
        for cell in c_generator:
            # 寻找距离当前 cell 距离为 stepSize 的轮廓点
...
def checkHairLOrR(cloth_image_input_cut,
    ...
    if threshold < 0 or threshold > 1: raise TypeError("阈值设置必须在 0 和 1 之间！")

    # 裁出 cloth_image_input_cut 按高度 40%～50% 的区域-cloth_image_input_cutbar，并转换为 A 矩阵，做二值化
    ...
    # 裁出 input_image、neck_image 的 A 矩阵的对应区域，并做二值化
    ...
    # 将三个 cutbar 合到一起-result_a_cutbar
...
def checkLongHair(neck_image, head_bottom_y, neck_top_y):
    """
    长发检测函数，输入为 head/neck 图像，通过下巴是否为最低点，来判断是否为长发
    :return 0 : 短发
    :return 1 : 长发
    """
