...
TIPS: `CUDA` と `CUDNN` をインストールした後、ローカルの onnxruntime パッケージのバージョンが互換性があるかどうかを確認する必要があります。最新版の cuda をインストールした場合、`pip install --upgrade onnxruntime`を実行するだけです。

TIPS2: torch をインストールする必要もあります。インストールする際、cuda のバージョンと互換性が必要です。cuda は下位互換性があります。たとえば、cuda のバージョンが 12.6 で、torch が対応する最高バージョンが 12.4 の場合、12.4 をあなたのコンピュータにインストールしても問題ありません。
