...
def cover_mask(image_path, mask_path, alpha=0.85, rate=0.1, if_save=True):
    ...
    在图片右下角盖上水印
    :param image_path:
    :param mask_path: 水印路径，以 PNG 方式读取
    :param alpha：不透明度，默认为 0.85
    :param rate：水印比例，越小水印也越小，默认为 0.1
    :param if_save: 是否将裁剪后的图片保存，如果为 True，则保存并返回新图路径，否则不保存，返回截取后的图片对象
    :return: 新的图片路径
    """
    ...
    # 我采取的策略是，先拷贝一张原图 im 为 base 作为基底，然后在 im 上利用 paste 函数添加水印
    # 此时的水印是完全不透明的，我需要利用 blend 函数内置参数 alpha 进行不透明度调整
    base = im.copy()
...
def get_box(image) -> list:
    ...
    flag1, flag2 = 0, 0
    box = [0, 0, 0, 0]  # 上下左右
    # 采用两面夹击战术，使用 flag1 和 2 确定两面的裁剪程度
    # 先得到上下
...
def filtering(img, f, x, y, x_max, y_max, x_min, y_min, area=0, noise_size=50) ->tuple:
    """
    filtering 将使用递归的方法得到一个连续图像（这个连续矩阵必须得是单通道的）的范围 (坐标)
    :param img: 传入的矩阵
    :param f: 和 img 相同尺寸的全零矩阵，用于标记递归递归过的点
    :param x: 当前递归到的 x 轴坐标
    :param y: 当前递归到的 y 轴坐标
    :param x_max: 递归过程中 x 轴坐标的最大值
    :param y_max: 递归过程中 y 轴坐标的最大值
    :param x_min: 递归过程中 x 轴坐标的最小值
    :param y_min: 递归过程中 y 轴坐标的最小值
    :param area: 当前递归区域面积大小
    :param noise_size: 最大递归区域面积大小，当 area 大于 noise_size 时，函数返回 (0, 1)
    :return: 分两种情况，当 area 大于 noise_size 时，函数返回 (0, 1)，当 area 小于等于 noise_size 时，函数返回 (box, 0)
            其中 box 是连续图像的坐标和像素点面积（上下左右，面积）
...
def zoom_image_without_change_size(image:np.ndarray, zoom_rate, interpolation=cv2.INTER_NEAREST) ->np.ndarray:
    ...
    :param image：传入的图像对象
    :param zoom_rate：放大比例，单位为倍（初始为 1 倍）
    :param interpolation：插值方式，与 opencv 的 resize 内置参数相对应，默认为最近邻插值
    :return: 裁剪后的图像实例
    """
    height, width, _ = image.shape
    if zoom_rate < 1:
        # zoom_rate 不能小于 1
        raise ValueError("zoom_rate 不能小于 1！")
...
def full_ties(image_pre:np.ndarray):
    ...
    def FillHole(image):
        ...
        seedPoint = (0, 0)
        # floodFill 函数中的 seedPoint 对应像素必须是背景
        is_break = False
        for i in range(im_floodFill.shape[0]):
            ...
            if (is_break):
                break
        # 得到 im_floodFill 255 填充非孔洞值
        cv2.floodFill(im_floodFill, mask, seedPoint, 255)
        # cv2.imshow("tmp1", im_floodFill)
        # 得到 im_floodFill 的逆 im_floodFill_inv
        im_floodFill_inv = cv2.bitwise_not(im_floodFill)
        # cv2.imshow("tmp2", im_floodFill_inv)
        # 把 image、im_floodFill_inv 这两幅图像结合起来得到前景
...
def cut_BiggestAreas(image):
    ...
    def find_BiggestAreas(image_pre):
        ...
        # 识别出最大的轮廓
        # 需要注意的是，在低版本的 findContours 当中返回的结果是 tuple，不支持 pop，所以需要将其转为 pop
...
def locate_neck(image:np.ndarray, proportion):
    """
    根据输入的图片 (四通道) 和 proportion(自上而下) 的比例，定位到相应的 y 点，然后向内收缩，直到两边的像素点不透明
    """
    if image.shape[-1] != 4:
        raise TypeError("请输入一张 png 格式的四通道图片！")
    if proportion > 1 or proportion <=0:
        raise ValueError("proportion 必须在 0~1 之间！")
...
def brightnessAdjustment(image: np.ndarray, bright_factor: int=0):
    ...
    :param bright_factor:亮度调节因子，可正可负，没有范围限制
            当 bright_factor ---> +无穷 时，图像全白
            当 bright_factor ---> -无穷 时，图像全黑
    :return: 处理后的图片
    """
...
def contrastAdjustment(image: np.ndarray, contrast_factor: int = 0):
    ...
    :param contrast_factor:亮度调节因子，可正可负，范围在 [-100, +100] 之间
            当 contrast_factor=-100 时，图像变为灰色
    :return: 处理后的图片
    """
...
class CV2Bytes(object):
    ...
    def byte_cv2(image_byte, flags=cv2.IMREAD_COLOR) ->np.ndarray:
        """
        将传入的字节流解码为图像，当 flags 为 -1 的时候为无损解码
        """
...
def comb2images(src_white:np.ndarray, src_black:np.ndarray, mask:np.ndarray) -> np.ndarray:
    """输入两张图片，将这两张图片根据输入的 mask 进行叠加处理
       这里并非简单的 cv2.add(),因为也考虑了羽化部分，所以需要进行一些其他的处理操作
       核心的算法为：dst = (mask * src_white + (1 - mask) * src_black).astype(np.uint8)

    Args:
        src_white (np.ndarray): 第一张图像，代表的是 mask 中的白色区域，三通道
        src_black (np.ndarray): 第二张图像，代表的是 mask 中的黑色区域，三通道
