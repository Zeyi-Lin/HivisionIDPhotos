...
class HivisionaiParams(object):
    ...
    # 腾讯云相关变量
    region = "ap-beijing"
    zip_key = "HY-sdk/"  # zip 存储的云端文件夹路径，这里改了 publish.yml 也需要更改
    # 云端用户配置，如果在 cloud_config_save 不存在，就需要下载此文件
...
class HivisionaiUtils(object):
    ...
    def download_dependence(path=None):
        """
        一键下载 HY-sdk 所需要的所有依赖，需要注意的是，本方法必须在运行 pip install 之后使用（运行完 pip install 之后才会出现 hivisionai 文件夹）
        Args:
            path: 文件路径，精确到 hivisionai 文件夹的上一个目录，如果为 None，则默认下载到 python 环境下 hivisionai 安装的目录

        Returns:
            下载相应内容到指定位置
        """
        # print("指定的下载路径：", path)  # 此时在 path 路径下必然存在一个 hivisionai 文件夹
        # print("系统安装的 hivisionai 库的路径:", HivisionaiParams.hivisionai_path)
...
class HivisionaiApps(object):
    ...
    def upgrade(v: str, enforce: bool = False, save_cached: bool = False):
        ...
        Args:
            ...
            save_cached: 是否保存下载的 wheel 文件，默认为否
        Returns:
            None
        """
        ...
        f.extractall(new_dir)  # 提取 zip 文件
    ...
    def export(path):
        """
        输出最新版本的文件到命令运行的 path 目录
        Args:
            path: 用户输入的路径

        Returns:
            输出最新的 hivisionai 到 path 目录
        """
        ...
        # 强制删除 bin/hivisionai 和 hivisionai/以及 HY_sdk-**
    ...
    def hy_func_init(force):
        """
        在 HY-func 目录下使用 hivisionai --init，可以自动将需要的依赖下载到指定位置
        不过对于比较大的模型——修复模型而言，需要手动下载
        Args:
            force: 如果 force 为 True，则会强制重新下载所有的内容，包括修复模型这种比较大的模型
        Returns:
        ...
        assert dirName == "HY-func", "请在正确的文件目录下初始化 HY-func!"
        # 需要下载的内容会存放在 HivisionaiParams 的 functionDependence 变量下
    ...
    def hy_func_deploy(functionName: str = None, functionPath: str = None):
        """
        在 HY-func 目录下使用此命令，并且随附功能函数的名称，就可以将 HY-func 的部署版放到桌面上
        但是需要注意的是，本方式不适合修复功能使用，修复功能依旧需要手动制作镜像
        Args:
            functionName: 功能函数名称
            functionPath: 需要注册的 HY-func 路径

        Returns:
            程序执行完毕，桌面会出现一个同名文件夹
        """
        # 为了代码撰写的方便，这里仅仅把模型文件删除，其余配置文件保留
        # 为了实现在任意位置输入 hivisionai --deploy funcName 都能成功，在使用前需要在.hivisionai/user_config.json 中注册
        # print(functionName, functionPath)
        if functionPath is not None:
            # 更新/添加路径
            # functionPath 为相对于使用路径的路径
            assert os.path.basename(functionPath) == "HY-func", "所指向路径非 HY-func!"
